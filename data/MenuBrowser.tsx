  ...
  return <>
    <section id="catalogue" className="scroll-mt-36">
      ...
      <div className="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">{displayedItems.map((item) => <button onClick={() => setSelected(item)} key={`${item.category}-${item.name}`} className="group text-center focus:outline-none">
        <div className="relative mx-auto aspect-square w-full overflow-hidden"><div className="absolute inset-8 rounded-full bg-tomato/10 blur-3xl"/><Image src={photoFor(item)} alt={item.name} fill sizes="(max-width: 640px) 100vw, 33vw" className={`object-contain p-4 drop-shadow-xl transition duration-500 group-hover:scale-105 ${item.category === "Pizzas" ? "mix-blend-multiply" : ""}`} />{item.popular && <span className="absolute right-3 top-3 rounded-full bg-gold px-3 py-1 text-xs font-black text-black">Populaire</span>}</div>
    ...
    {selected && <div role="dialog" aria-modal="true" aria-label={selected.name} onClick={() => setSelected(null)} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm">
      <article onClick={(e) => e.stopPropagation()} className="relative max-h-[94vh] w-full max-w-4xl overflow-auto rounded-[2rem] bg-white shadow-2xl">
        ...
        <div className="grid md:grid-cols-[1.05fr_.95fr]"><div className="relative min-h-80 overflow-hidden bg-white md:min-h-[560px]"><Image src={photoFor(selected)} alt={selected.name} fill sizes="(max-width: 768px) 100vw, 55vw" className={`object-contain p-8 ${selected.category === "Pizzas" ? "mix-blend-multiply" : ""}`} /></div><div className="flex flex-col justify-center bg-white p-8 md:p-12"><p className="font-bold uppercase tracking-widest text-tomato">{selected.category}</p><h2 className="mt-2 text-4xl font-black">{selected.name}</h2><p className="mt-6 text-xl italic leading-relaxed text-black/65">{selected.description}</p><p className="mt-7 text-2xl font-black text-tomato">{selected.price}</p><a href="tel:+33134121211" className="mt-8 rounded-full bg-tomato px-6 py-4 text-center font-black text-white">Commander au 01 34 12 12 11</a></div></div>
      </article>
    </div>}
  </>;
}
